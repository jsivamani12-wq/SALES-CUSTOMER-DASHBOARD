# SALES-CUSTOMER-DASHBOARD
Interactive Sales &amp; Customer Analytics Dashboards built in Tableau | 2020-2023 Superstore Data  
• Sales Performance Dashboard with YoY trends 
• Customer Behavior &amp; Segmentation Dashboard   
• 4 cleaned tables (Orders, Customer, Location, Product) processed in Power Query


# Sales & Customer Analytics Dashboards | Tableau

**Interactive Sales Performance & Customer Analytics Dashboards** built in Tableau using 4 years of Superstore transaction data (2020–2023). 

This project demonstrates end-to-end data analytics skills — from **data cleaning with Power Query** to building professional, insightful, and interactive dashboards.

![Sales & customer Dashboard 2020-2023 Preview] <a href="https://github.com/jsivamani12-wq/SALES-CUSTOMER-DASHBOARD/commit/6322ec74b7a6c8cecb3049256e1537ea01a5ad92"> click..me </a>
![Sales & customer Dashboard 2020-2023 Black-Theam Preview] <a href="https://github.com/jsivamani12-wq/SALES-CUSTOMER-DASHBOARD/commit/9ec7f2fc76e59da6aab02d2de21b09d93d8125b9"> click..me </a>
## Project Highlights

- **Two Professional Dashboards**: Sales Performance + Customer Analytics
- **Data Period**: 2020 to 2023 (4 years of transactions)
- **Data Model**: 4 cleaned tables — **Orders, Customer, Location, Product**
- **Data Cleaning**: Thoroughly cleaned and transformed using **Power Query**
- Original YouTube tutorial + **Two custom variations** with modern dark theme and enhanced interactivity

## Dashboards Included

1. **Sales Dashboard** (2023 & 2022 views)
   - KPIs with YoY growth (Sales, Profit, Quantity)
   - Monthly trends with highest/lowest month indicators
   - Sub-category performance comparison
   - Weekly sales & profit trends

2. **Customer Dashboard**
   - Customer segmentation and behavior analysis
   - Sales contribution by customers
   - High-value vs low-value customer insights
   - Purchase patterns and concentration analysis

## Skills Demonstrated

- Advanced Tableau visualizations and dashboard design
- Year-over-Year (YoY) growth calculations
- Data modeling with multiple related tables
- Professional dashboard layout and UI/UX (Dark theme)
- Data cleaning & transformation using Power Query
- Calculated fields, parameters, sets, and dashboard actions

## Live Interactive Dashboards 

**Tableau Public Links:**
- [Tablue public.com] < a href="https://public.tableau.com/app/discover"> public.tablue.com </a>
- [Tablue public-sales & customer dashboard]  <a href="https://public.tableau.com/app/profile/siva.mani8858/viz/SALESPROJECT-MAIN-WHITE/SALSEDashboard1"> Dashboard </a>
- [Tablue public-sales & customer dashboard Black Team] <a href="https://public.tableau.com/app/profile/siva.mani8858/viz/SALESPROJECT-MAIN-BLACK/SALSEDashboard1?publish=yes"> dashboard</a>

**Download Tableau Workbook (.twbx):**
- [Download Full Sales & Customer-white Workbook] <a href="https://github.com/jsivamani12-wq/SALES-CUSTOMER-DASHBOARD/blob/main/SALES%20PROJECT-MAIN-WHITE.twbx">
  Dashboard </a>
- [Download Full Sales & Customer-Black Theam Workbook] <a href="https://github.com/jsivamani12-wq/SALES-CUSTOMER-DASHBOARD/blob/main/SALES%20PROJECT-MAIN%20-%20BLACK.twbx"> Dashboard </a>
## Screenshots

![Sales & CustomerDashboard 2020-2023]- <a href="https://github.com/jsivamani12-wq/SALES-CUSTOMER-DASHBOARD/commit/6322ec74b7a6c8cecb3049256e1537ea01a5ad92"> click..me </a>
![Sales & Customer Dashboard Black Theam 2022] <a href="https://github.com/jsivamani12-wq/SALES-CUSTOMER-DASHBOARD/commit/9ec7f2fc76e59da6aab02d2de21b09d93d8125b9"> click..me </a>

## Dataset & Data Preparation

- **Dataset**: Superstore Sales Dataset
- **Tables Used**: Orders, Customer, Location, Product (4 tables)
- **Cleaning Tool**: Power Query (Excel/Power BI)
- **Steps Done**: Removed duplicates, handled missing values, standardized formats, created proper relationships
## Data sets link
Orders,Sales,Product,Location Cleand Data-sets- <a href ="https://github.com/jsivamani12-wq/SALES-CUSTOMER-DASHBOARD"> click..me </a>
## Key Insights Delivered

- 2023 achieved **$733K** in Total Sales with **20.36%** YoY growth
- Identified top-performing and under-performing sub-categories
- Highlighted high-value customers and sales concentration patterns
- Discovered clear seasonal and weekly trends

## Future Enhancements (In Progress)

- RFM Customer Segmentation
- Sales Forecasting
- Integration with HR Analytics Dashboard

## Connect With Me

- **LinkedIn**: 
- **Tableau Public Profile**: < a href="https://public.tableau.com/app/profile/siva.mani8858/vizzes"> click..me </a>
- **Email**: jsivamnai12@gmail.com

---

**Made with ❤️ using Tableau & Power Query**

---
